...
class Aluno;
class List; // lista ordenada versao 2
class Node;
...
bool List::push_ord(Aluno* obj)
{
	...
	Node** p= &head;
	while(*p && obj->get_Mat() > ((*p)->D).get_Mat()) // ordem crescente
	{
		p= &((*p)->next);
	}
	aux->next= *p;
...
int main ()
{
	...
	while(op>=1&&op<=7)
	{	
		cout << "1-Cadastrar.\n2-Retirar.\n3-Mostrar item.\n4-Mostrar tudo\n5-Quantidade.\n6-Verifica vazio.\n7-Retira quem e apontado por head\nOutro numero - sair.\nCodigo: ";
		cin >> op;
		cout << "\n";
		switch(op)
		{
